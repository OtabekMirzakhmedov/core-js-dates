...
function formatDate(date) {
  ...
  const hour = newdate.getUTCHours() % 12 || 12;
  const amPm = newdate.getUTCHours() < 12 ? 'AM' : 'PM';

  const formattedDate = `${month}/${day}/${year}, ${hour}:${newdate.getUTCMinutes().toString().padStart(2, '0')}:${newdate.getUTCSeconds().toString().padStart(2, '0')} ${amPm}`;
...
function getWeekNumberByDate(date) {
  ...
  const daysPassed = Math.floor((date - januaryFirst) / (24 * 60 * 60 * 1000));
  const firstDayOfWeek = januaryFirst.getDay();
  let weekNumber = Math.floor((daysPassed + firstDayOfWeek) / 7) + 1;
  if (firstDayOfWeek === 0) {
    weekNumber += 1;
  }
  if (date.getDay() === 0) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const [day, month, year] = period.end.split('-').map(Number);
  const startDate = new Date(period.start);
  ...
  return schedule.map((date) => formatDate2(date));
}
